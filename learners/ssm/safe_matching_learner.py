...
from learners.ssm.diffusion import (
    DDPM,
    FourierFeatures,
...
from learners.ssm.mlp import MLP
from learners.ssm.state_action_value import StateActionValue


DatasetDict = Dict[str, jnp.ndarray]
...
class SafeScoreMatchingLearner:
    ...
    def create(
        ...
        safety_base_cls = partial(MLP, hidden_dims=safety_hidden_dims, activate_final=True)
    ...
    def act(self, obs: np.ndarray, deterministic: bool = False, rng=None):
        ...
        agent = self
        if rng is not None:
            agent = agent.replace(rng=jax.random.PRNGKey(rng) if isinstance(rng, int) else rng)
        if deterministic:
            actions, agent = agent.eval_actions(obs_arr)
        else:
            actions, agent = agent.sample_actions(obs_arr)
        return np.asarray(actions), agent, {}

    def update_q(self, batch: DatasetDict) -> Tuple["SafeScoreMatchingLearner", Dict[str, float]]:
        agent = self
        ...
        target_critic_1_params = optax.incremental_update(critic_1.params, agent.target_critic_1.params, agent.tau)
        target_critic_2_params = optax.incremental_update(critic_2.params, agent.target_critic_2.params, agent.tau)
    ...
    def _safety_targets(self, agent: "SafeScoreMatchingLearner", batch: DatasetDict, rng: jax.random.PRNGKey):
        next_actions, rng = ddpm_sampler(
            agent.score_model.apply_fn,
            agent.score_model.params,
            agent.T,
    ...
    def update_safety(self, batch: DatasetDict) -> Tuple["SafeScoreMatchingLearner", Dict[str, float]]:
        agent = self
        rng = agent.rng
        ...
        grads, metrics = jax.grad(safety_loss_fn, has_aux=True)(agent.safety_critic.params)
        safety_critic = agent.safety_critic.apply_gradients(grads=grads)
        target_params = optax.incremental_update(safety_critic.params, agent.target_safety_critic.params, agent.safety_tau)
        ...
        metrics["qh_target_stage_mean"] = jnp.maximum(0.0, batch["costs"] - agent.cost_limit).mean()
    ...
    def update_actor(self, batch: DatasetDict) -> Tuple["SafeScoreMatchingLearner", Dict[str, float]]:
        agent = self
    ...
    def save(self, ckpt_dir: str, step: Optional[int] = None):
        ckpt_path = f"{ckpt_dir}/ssm_ckpt" + (f"_{step}" if step is not None else "")
        data = serialization.to_bytes(self)
        with open(ckpt_path, "wb") as f:
            f.write(data)

    def load(self, ckpt_dir: str, step: Optional[int] = None):
        ckpt_path = f"{ckpt_dir}/ssm_ckpt" + (f"_{step}" if step is not None else "")
        with open(ckpt_path, "rb") as f:
            data = f.read()
        return serialization.from_bytes(type(self), self, data)

    def get_weights(self):
        return serialization.to_state_dict(self)

    def set_weights(self, state_dict):
        return serialization.from_state_dict(type(self), self, state_dict)
...
def prepare_batch(batch: Dict[str, Union[np.ndarray, jnp.ndarray]]) -> DatasetDict:
    return {k: _to_float_array(v) for k, v in batch.items()}
