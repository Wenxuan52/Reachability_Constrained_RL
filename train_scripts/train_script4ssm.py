...
def parse_args():
    ...
    parser.add_argument("--replay_starts", type=int, default=None)
    ...
    if args.max_buffer_size is None:
        args.max_buffer_size = args.buffer_size
    if args.replay_starts is None:
        args.replay_starts = args.start_training
    if args.replay_batch_size is None:
        args.replay_batch_size = args.batch_size
    return args
...
def to_tf_batch(batch):
    if len(batch) == 7:
        ...
        # When the replay buffer appends indices, ignore the trailing entry
        obs, act, rew, next_obs, done, cost, _, _ = batch
...
def _unwrap_obs(obs):
    """Handle Gym reset/step outputs that may return (obs, info)."""
...
def main():
    ...
    if args.mode == "training":
        ...
        logging.info("Starting training. Logs will be written to %s", log_dir)
    else:
        logging.info("Running evaluation from %s", log_dir)
    ...
    buffer._maxsize = args.max_buffer_size
    buffer.replay_batch_size = args.replay_batch_size
    buffer.replay_starts = args.replay_starts
    logging.info(
        "Buffer ready (capacity=%d, start_training=%d). Beginning interaction...",
        buffer._maxsize,
        args.start_training,
    )
    ...
    for step in range(1, args.total_steps + 1):
        if step < args.start_training:
            ...
            action, _ = learner.act(np.asarray(obs, dtype=np.float32))
            action = np.asarray(action)
        ...
        if step % args.log_interval == 0:
            writer.flush()
            logging.info(
                "[train] step=%d return=%.2f cost=%.2f episode_len=%d buffer_size=%d",
                step,
                episode_return,
                episode_cost,
                episode_len,
        ...
        if step % args.eval_interval == 0:
            ...
            writer.flush()
            logging.info(
                "[eval] step=%d return=%.2f violation=%.2f",
                step,
                eval_metrics.get("eval/episode_return", 0.0),
                eval_metrics.get("eval/constraint_violation", 0.0),
            )
